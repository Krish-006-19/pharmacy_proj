...
router.get('/',async(req, res)=>{
    try {
        ...
        if(!orders){
            return res.status(200).json({ success : true, message : "No Orders Yet ! "});
        }
        return res.status(200).json({ orders : orders , success : true});
    } catch (error) {
        return res.status(200).json({ message : "Something Went Wrong in finding Orders" , success : false});
    }
})
...
router.post('/placeOrder/:pharmacyId/:supplierId', async (req, res) => {
  ...
  if (!pharmacyId || !supplierId || !Array.isArray(medicines) || medicines.length === 0)
    return res.status(400).json({ success: false, message: 'Invalid payload' });

  const idsToValidate = [pharmacyId, supplierId, ...medicines.map(m => m.medicine)];
